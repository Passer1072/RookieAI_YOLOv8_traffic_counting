...
# 定义识别线线的起点和终点坐标
line_start = np.array([250, 500])  # 起点
line_end = np.array([850, 500])  # 终点

counter = 0  # 计数器
counter_id = 0  # 分配唯一 ID 的计数器
buffer_tracks = {}  # 保存位置的元组
...
def main():
    ...
    while True:
        ...
        for r in results:
            ...
            cv2.putText(frame_, FPS_str, (5, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            ...
            for box in boxes:
                ...
                for id, past_positions in buffer_tracks.items():
                    if len(past_positions) > 0 and np.linalg.norm(
                            past_positions[-1] - center) < identification_range:  # 如果 y 位置接近缓冲区中的 1
                        box_id = id
                        break
                '''
                ...
                # 更新 buffer_tracks
                buffer_tracks[box_id].append(center)

                # 检查框是否越界
                if len(buffer_tracks[box_id]) > 1:
                    if (buffer_tracks[box_id][-2][1] - line_start[1]) * (
                            buffer_tracks[box_id][-1][1] - line_start[1]) < 0:
                        # 盒子已经越界了
        ...
        cv2.putText(frame_, counter_str, (5, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)  # 将计数器放在框架上
